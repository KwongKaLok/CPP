...
void mlfq()
{
    ...
    struct Queue q0; // RR with tq0
    struct Queue q1; // RR with tq1
    struct Queue q2; // FCFS
    queue_init(&q0);
    queue_init(&q1);
    queue_init(&q2);
    ...
    int remain_tq0 = tq0;
    int remain_tq1 = tq1;
    while (clock <= MAX_GANTT_CHART) // clock
    {
        if (!queue_is_full(&q0) && process_table[next_process].arrival_time <= clock && next_process < process_table_size)
        {
            queue_enqueue(&q0, next_process);
            process_table[next_process].remain_time = process_table[next_process].burst_time;
            next_process++;
        }
        clock++;
        if (!queue_is_empty(&q0))
        {
            current_process = queue_peek(&q0);

            if (process_table[current_process].remain_time != 0 && remain_tq0 != 0)
            {
            ...
            {
                queue_dequeue(&q0);
                queue_enqueue(&q1, current_process);
                remain_tq0 = tq0;
            }
            if (process_table[current_process].remain_time == 0 && remain_tq0 >= 0)
            {
        ...
        if (queue_is_empty(&q0) && !queue_is_empty(&q1))
        {
            current_process = queue_peek(&q1);
            if (process_table[current_process].remain_time != 0 && remain_tq1 != 0)
            {
                process_table[current_process].remain_time--;
                remain_tq1--;
                gantt_chart_update(chart, &sz_chart, process_table[current_process].name, 1);
            ...
            {
                queue_dequeue(&q1);
                queue_enqueue(&q2, current_process);
                remain_tq1 = tq1;
            }
            if (process_table[current_process].remain_time == 0 && remain_tq1 >= 0)
            {
        ...
        if (queue_is_empty(&q0) && queue_is_empty(&q1) && !queue_is_empty(&q2))
        {
            current_process = queue_peek(&q2);
